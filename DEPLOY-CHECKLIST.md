# Deploy Checklist — ร้านรับซื้อไอโฟน.com (Coolify)
...
- [ ] ตั้ง **โดเมน + SSL** ให้ชี้มาที่ Coolify และรองรับ IDN (`ร้านรับซื้อไอโฟน.com`) / punycode ตาม DNS

## SEO

- [ ] `public/robots.txt` — `Allow: /` และ `Sitemap` ชี้ `https://ร้านรับซื้อไอโฟน.com/sitemap-index.xml`
...
- [ ] หน้าแรกมี CTA LINE ชัด (Hero / Sticky mobile / Premium LINE / Footer) และ GEO internal links ไปกรุงเทพ–อีสาน
...
```bash
curl -sS -D - "https://ร้านรับซื้อไอโฟน.com/" -H "Accept: text/markdown" -o /dev/null
```
...
- [ ] มี `/พื้นที่ให้บริการ/` และหน้า location ที่มีใน `src/content/locations/` แล้ว
